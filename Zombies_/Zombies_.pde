...
ArrayList<float[]> bullet(float x, float y, int mouseX, int mouseY,float damage, ArrayList<float[]>zombies) {//everything that is run when firing
...
int wS=0; //weapon selection. current held gun.
float[] xyt=new float[3];
...
void draw() {
  ...
  if (mousePressed){
    ...
    if(fireRateTimer<=0){
      zombies=bullet(playerX, playerY, mouseX, mouseY,, zombies);//shoot and test if it hits etc
