...
void zombie() {//auto places a zombie with 100 health at a random wall at a random point on that wall
  ...
  {//used to get x,y,health,speed of zombies
...
void mousePressed() {
  if (menu) {
    ...
    for (int i=0; i<guns.length; i++) {
      if (button(250, i*60+20, 385, i*60+70, mouseX, mouseY)&&money>=guns[i][7]&&guns[i][0]==0) {
...
void draw() {
  ...
  if (menu) {
    ...
    for (int i=0; i<guns.length; i++) {
      fill(50, 50, 155);
      rect(250, i*60+20, 140, 50, 10);
      fill(255);
      text(wNames[i], 255, i*60+40);
